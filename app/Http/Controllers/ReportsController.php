...
class ReportsController extends Controller
{
    ...
    public function salesReport(Request $request)
    {
        ...
        $byStore = $byStore->map(function ($row) {
            $row->store_name = $this->allStoreLocations[$row->store] ?? $row->store;
            return $row;
        });
        ...
        $data = [
            ...
            'allStoreLocations'  => $this->allStoreLocations,  // ← required by blade
        ];
    ...
    public function paymentReport(Request $request)
    {
        ...
        for ($hour = 0; $hour < 24; $hour++) {
            $totalSales = $allPaymentModes->sum(function ($mode) use ($byHourMode, $hour) {
                return (float) ($byHourMode[$mode]->keyBy('hour')[$hour]->sales ?? 0);
            });
            $totalOrders = $allPaymentModes->sum(function ($mode) use ($byHourMode, $hour) {
                return (int) ($byHourMode[$mode]->keyBy('hour')[$hour]->orders ?? 0);
            });
        ...
        // ✅ Daily Sales Trend by Payment Mode
        ...
        foreach ($period as $date) {
            $day         = $date->toDateString();
            $totalSales  = $paymentModes->sum(fn($m) => (float) ($salesByDayModeRaw[$m]->keyBy('day')[$day]->sales ?? 0));
            $totalOrders = $paymentModes->sum(fn($m) => (int) ($salesByDayModeRaw[$m]->keyBy('day')[$day]->orders ?? 0));
    ...
    public function ordersReport(Request $request)
    {
        ...
        // 👔 Scope query to the user's accessible stores via LocationConfig
        if ($user->user_location) {
            $regionStores = LocationConfig::regionStores($user->user_location);
            if (!empty($regionStores)) {
                $query->whereIn('requesting_store', $regionStores);
            } else {
        ...
        $allStoreLocations = $this->allStoreLocations;
        $storeLocations    = LocationConfig::accessibleStores($user->role, $user->user_location);

        return view('reports.orders_report', compact(
            'orders',
            'channels',
            'statuses',
            'perPage',
    ...
    public function exportOrdersReport(Request $request)
    {
        ...
        // 👔 Scope to user's accessible stores + manager status restriction
        if ($user->user_location) {
            $regionStores = LocationConfig::regionStores($user->user_location);
            if (!empty($regionStores)) {
                $query->whereIn('requesting_store', $regionStores);
        ...
        foreach ($orders as $order) {
            ...
            fputcsv($handle, [
                ...
                $order->customer_name,
                $order->channel_order,
                Carbon::parse($order->time_order)->format('Y-m-d'),
                $storeName,
    ...
    public function exportCsv(Request $request)
    {
        ...
        // Single source of truth via LocationConfig
        ...
        $callback = function () use ($orders, $selectedColumns, $columnHeaders, $stores) {
            ...
            $totalsRow = [];
            foreach ($selectedColumns as $col) {
                if ($col === 'grand_total')    $totalsRow[] = number_format($grandTotalSum, 2);
                elseif ($col === 'total_payable')  $totalsRow[] = number_format($payableSum, 2);
                elseif ($col === 'total_freebies') $totalsRow[] = number_format($freebiesSum, 2);
                else                               $totalsRow[] = '';
            }
