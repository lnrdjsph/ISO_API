...
class FormsController extends Controller
{
    ...
    public function sof()
    {
        ...
        // Resolve user warehouse — supports both store code AND region key
        ...
        $warehouseMap = LocationConfig::warehouses(); // ['80141' => 'Silangan Warehouse', ...]
    ...
    public function sof_submit(Request $request)
    {
        ...
        foreach ($request->input('orders', []) as $index => $order) {
            validator(
                ...
                [
                    ...
                    'price_per_pc.required' => "Price per Piece is required for item no. " . ($index + 1),
                    'price_per_pc.numeric' => "Price per Piece must be a number for item no. " . ($index + 1),
                    'price.required' => "Price is required for item no. " . ($index + 1),
                    'price.numeric' => "Price must be a number for item no. " . ($index + 1),
                    'qty_per_cs.required' => "Cases Ordered is required for item no. " . ($index + 1),
                    'qty_per_cs.integer' => "Cases Ordered must be a whole number for item no. " . ($index + 1),
                    'qty_per_pc.required' => "Quantity per Piece is required for item no. " . ($index + 1),
                    'qty_per_pc.integer' => "Quantity per Piece must be a whole number for item no. " . ($index + 1),
        ...
        try {
            ...
            $currentMonth = now()->format('Ym'); // YYYYMM
            $latest = Order::where('sof_id', 'like', "SOF{$currentMonth}-%")
                ->orderBy('sof_id', 'desc')
                ->first();
