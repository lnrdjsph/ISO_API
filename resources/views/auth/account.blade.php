...
<div class="mx-auto max-w-3xl px-4 py-8">
    ...
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <form method="POST" action="{{ route('account.update') }}" class="px-6 py-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name"
                           ...
                           class="w-full rounded-lg border @error('name') border-red-400 bg-red-50 @else border-gray-300 @enderror px-4 py-2.5 text-sm shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                           required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email"
                           ...
                           class="w-full rounded-lg border @error('email') border-red-400 bg-red-50 @else border-gray-300 @enderror px-4 py-2.5 text-sm shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                           required>
                    @error('email')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <div id="password-section"
            ...
            <form method="POST" action="{{ route('account.password') }}" class="px-6 py-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" name="current_password" id="current_password"
                               class="w-full rounded-lg border @error('current_password') border-red-400 bg-red-50 @else border-gray-300 @enderror px-4 py-2.5 pr-10 text-sm shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                               placeholder="Enter current password" autocomplete="current-password">
                        <button type="button" onclick="toggleVis('current_password')"
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="password" name="password" id="password"
                                   class="w-full rounded-lg border @error('password') border-red-400 bg-red-50 @else border-gray-300 @enderror px-4 py-2.5 pr-10 text-sm shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                                   placeholder="Minimum 6 characters" autocomplete="new-password">
                            <button type="button" onclick="toggleVis('password')"
                                    class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                     viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="mt-2">
                            <div class="h-1.5 w-full rounded-full bg-gray-200">
                                <div id="strength-bar"
                                     class="h-1.5 rounded-full bg-gray-300 transition-all duration-300"
                                     style="width:0%"></div>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" onclick="toggleVis('password_confirmation')"
                                    class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                     viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <p id="match-label" class="mt-1 text-xs text-gray-400 hidden"></p>
                    </div>
...
<script>
    // Toggle password section accordion
    ...
    // Auto-open if errors exist in password section
    document.addEventListener('DOMContentLoaded', function () {
        const section = document.getElementById('password-section');
        if (!section.classList.contains('hidden')) {
            document.getElementById('password-chevron').style.transform = 'rotate(180deg)';
        }
    });
    ...
    pwInput.addEventListener('input', function () {
        ...
        const levels = [
            { w: '0%',   c: 'bg-gray-300',   l: 'Enter a password', lc: 'text-gray-400'   },
            { w: '25%',  c: 'bg-red-400',     l: 'Weak',             lc: 'text-red-500'    },
            { w: '50%',  c: 'bg-yellow-400',  l: 'Fair',             lc: 'text-yellow-600' },
            { w: '75%',  c: 'bg-blue-400',    l: 'Good',             lc: 'text-blue-600'   },
            { w: '90%',  c: 'bg-green-400',   l: 'Strong',           lc: 'text-green-600'  },
            { w: '100%', c: 'bg-green-500',   l: 'Very Strong',      lc: 'text-green-700'  },
        ];
    ...
    function checkMatch() {
        ...
        if (!cpw) { matchLbl.classList.add('hidden'); return; }
