            ...
            <div class="overflow-hidden rounded-xl border border-amber-200 bg-white shadow-sm">
                ...
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-amber-100 px-5 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-sm font-semibold text-gray-900">Stuck Orders</h2>
                        <span class="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700">
                            {{ $stuckOrders->count() }}
                        </span>
                    </div>
                    <a href="{{ route('orders.index') }}" class="text-xs text-gray-400 hover:text-gray-600">View all →</a>
                </div>
                ...
                <div class="space-y-2 p-4 sm:hidden">
                    @foreach ($stuckOrders as $stuck)
                        @php
                            $isUrgent = $stuck->days_stuck >= 5;
                            $cls = $statusBadgeClasses[strtolower($stuck->order_status)] ?? 'bg-gray-100 text-gray-600';
                        @endphp
                        <a href="{{ route('orders.show', $stuck->id) }}"
                            class="{{ $isUrgent ? 'border-red-200 bg-red-50/30' : 'border-amber-100' }} block rounded-lg border p-3 transition hover:shadow-sm">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-2">
                                        <span class="font-mono text-sm font-medium text-gray-900">{{ $stuck->sof_id }}</span>
                                        <span class="{{ $cls }} rounded-full px-1.5 py-0.5 text-[9px] font-medium">
                                            {{ ucwords($stuck->order_status) }}
                                        </span>
                                    </div>
                                    <p class="mt-1 truncate text-xs text-gray-500">{{ $stuck->customer_name ?? '—' }}</p>
                                    <div class="mt-2 flex items-center gap-3 text-xs">
                                        <span class="{{ $isUrgent ? 'text-red-600' : 'text-amber-600' }} flex items-center gap-1">
                                        ...
                                        <span class="text-gray-500">{{ $stuck->reminder }}</span>
                                    </div>
                                </div>
                ...
                <div class="hidden overflow-x-auto sm:block">
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 text-left text-[10px] uppercase text-gray-400">
                                <th class="px-5 py-2 font-medium">SOF ID</th>
                                <th class="px-4 py-2 font-medium">Customer</th>
                                @if ($isAdmin || $isMgr)
                                    <th class="px-4 py-2 font-medium">Store</th>
                                @endif
                                <th class="px-4 py-2 font-medium">Status</th>
                                <th class="px-4 py-2 font-medium">Stuck</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach ($stuckOrders as $stuck)
                                @php
                                    ...
                                    $cls = $statusBadgeClasses[strtolower($stuck->order_status)] ?? 'bg-gray-100 text-gray-600';
                                @endphp
                                <tr class="{{ $isUrgent ? 'bg-red-50/20' : '' }} transition hover:bg-gray-50">
                                    <td class="whitespace-nowrap px-5 py-2 font-mono text-xs font-medium text-gray-900">
                                        {{ $stuck->sof_id }}
                                    </td>
                                    <td class="max-w-[150px] truncate px-4 py-2 text-xs text-gray-600">
                                        {{ $stuck->customer_name ?? '—' }}
                                    </td>
                                    @if ($isAdmin || $isMgr)
                                        <td class="max-w-[120px] truncate px-4 py-2 text-xs text-gray-500">
                                            {{ $stuck->store_name ?? '—' }}
                                        </td>
                                    @endif
                                    <td class="whitespace-nowrap px-4 py-2">
                                        <span class="{{ $cls }} rounded-full px-2 py-0.5 text-[9px] font-medium">
                                            {{ ucwords($stuck->order_status) }}
                                        </span>
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-2">
                                        <span class="{{ $isUrgent ? 'text-red-600 font-medium' : 'text-amber-600' }} text-xs">
                                            {{ $stuck->days_stuck }}d
                                        </span>
                                    </td>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2">
                                        <a href="{{ route('orders.show', $stuck->id) }}" class="text-xs text-amber-600 hover:text-amber-800">Review →</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        {{-- @if ($isStore)
            @php
                $pipeline = [
                    ...
                    ['For Approval', 'for approval', $pipelineCounts['for_approval_count'] ?? 0, 'bg-purple-500', 'text-purple-700', 'bg-purple-50'],
                    ['Approved', 'approved', $pipelineCounts['approved_count'] ?? 0, 'bg-emerald-500', 'text-emerald-700', 'bg-emerald-50'],
                    ['Completed', 'completed', $pipelineCounts['completed_count'] ?? 0, 'bg-teal-500', 'text-teal-700', 'bg-teal-50'],
                ];
                if (($pipelineCounts['rejected_count'] ?? 0) > 0) {
                    $pipeline[] = ['Rejected', 'rejected', $pipelineCounts['rejected_count'] ?? 0, 'bg-orange-500', 'text-orange-700', 'bg-orange-50'];
                }
                if (($pipelineCounts['cancelled_count'] ?? 0) > 0) {
                    $pipeline[] = ['Cancelled', 'cancelled', $pipelineCounts['cancelled_count'] ?? 0, 'bg-gray-400', 'text-gray-600', 'bg-gray-50'];
                }
            @endphp
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <div class="border-b border-gray-100 px-5 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-sm font-semibold text-gray-900">Order Pipeline</h2>
                    </div>
                </div>

                <div class="overflow-x-auto p-4">
                    <div class="flex items-center gap-2">
                        @foreach ($pipeline as $i => [$label, $statusKey, $cnt, $dot, $text, $bg])
                            @if ($i > 0 && $i <= 4)
                            ...
                            @if ($i == 4 && count($pipeline) > 4)
                                <div class="mx-1 flex-shrink-0 text-lg font-light text-gray-300" aria-hidden="true">|</div>
                            @endif

                            <a href="{{ route('orders.index', ['status' => $statusKey]) }}"
                                class="{{ $bg }} group flex flex-shrink-0 items-center gap-2 rounded-lg px-3 py-2 transition hover:shadow-md">
                                <span class="{{ $dot }} flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white shadow-sm">
                                    {{ number_format($cnt) }}
                                </span>
                                <span class="{{ $text }} whitespace-nowrap text-xs font-medium">{{ $label }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
        ...
        {{-- @if (!$isAdmin && ($isMgr || $isStore) && $statusBreakdown->isNotEmpty())
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 px-5 py-3">
                    <h2 class="text-sm font-semibold text-gray-800">Order Status Distribution</h2>
                    <span class="text-[10px] text-gray-400">all-time</span>
                </div>
                <div class="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3 lg:grid-cols-4">
                    @foreach ($statusBreakdown as $label => $meta)
                        @php $pct = $totalOrders > 0 ? round(($meta['count'] / $totalOrders) * 100) : 0; @endphp
                        <div class="flex items-center gap-3 rounded-xl border border-gray-200 px-3 py-2.5">
                            <span class="{{ $meta['color'] }} h-2.5 w-2.5 flex-shrink-0 rounded-full"></span>
                            <div class="min-w-0">
                                <p class="truncate text-[11px] text-gray-500">{{ $label }}</p>
                                <p class="text-lg font-bold leading-tight text-gray-900">
                                    {{ number_format($meta['count']) }}
                                    <span class="text-[10px] font-normal text-gray-400">({{ $pct }}%)</span>
                                </p>
                            </div>
                        </div>
                    @endforeach
