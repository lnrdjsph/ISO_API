@props([
    ...
    'caption' => null,
    'alt' => null,
    'height' => 'h-100', // tailwind height utility for the image frame
...
@if ($placeholder)
    ...
    <div class="mt-4 overflow-hidden rounded-lg border-2 border-dashed border-gray-300 bg-gray-50/60">
        <div class="{{ $height }} flex flex-col items-center justify-center gap-2 text-center">
...
@else
    <div class="screenshot-frame mt-4 overflow-hidden rounded-lg bg-gray-50 ring-1 ring-gray-200">
        <div class="{{ $height }} flex items-center justify-center overflow-hidden">
            <img class="max-h-full max-w-full object-contain" src="{{ asset('images/guide/' . $src) }}" alt="{{ $alt ?? ($caption ?? 'Screenshot') }}">
        </div>
        @if ($caption)
